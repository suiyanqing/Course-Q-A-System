...
if (a.equals("insert")) {
    // 执行插入课程分类模块,表名：kechengfenlei
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据

    String charuid = new CommDAO().insert(request, "kechengfenlei", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    // 插入数据完成，弹出页面并跳转到指定页面
    ...
    // 执行更新课程分类模块,表名：kechengfenlei
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据

    String charuid = new CommDAO().update(request, "kechengfenlei", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    Map map = Query.make("kechengfenlei").find(Request.get("id"));

    new CommDAO().delete(request, "kechengfenlei"); // 执行删除命令
