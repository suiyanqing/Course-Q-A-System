...
<%
    ...
    if (a.equals("insert")) {
        ...
        String charuid = new CommDAO().insert(request, "wentitiwen", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
        new CommDAO().commOper("update yonghu set tiwenshu=tiwenshu+1 where yonghuming='" + request.getParameter("tiwenren") + "'");
        ...
        // 执行更新问题提问模块,表名：wentitiwen
        HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
        ext.put("neirong", Info.download(Request.get("neirong"))); // 将前台编辑器内容的图片远程下载下来并保存在本地中
        String charuid = new CommDAO().update(request, "wentitiwen", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
        ...
        // 获取一行数据
        Map map = Query.make("wentitiwen").find(Request.get("id"));

        new CommDAO().delete(request, "wentitiwen"); // 执行删除命令
